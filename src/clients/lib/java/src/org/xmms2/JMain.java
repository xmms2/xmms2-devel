...
public final class JMain extends Thread{
	...
	/**
	 * The run method - our little mainloop. First call SpecialJNI.setENV(this), which is a native method and sets "this"
	 * as GlobalRef for the callback-oject in c and sets the pointer jvm, which is needed to call java methods outside
	 * the JNI functions. Furthermore, init the callback-methods (set xmms2d's result_notifiers). While the file-descriptor 
	 * is valid, we will wait for new input and if something is available, call Xmmsclient.xmmsc_io_in_handle(myConnection)
	 * 
	 */
	public void run(){
		FileDescriptor fd = new FileDescriptor();
		SpecialJNI.getFD(fd, Xmmsclient.getPointerToConnection(myConnection));
		SpecialJNI.setupMainloop(this, Xmmsclient.getPointerToConnection(myConnection));
