...
extern xmmsv_t *xmms_medialib_query_to_xmmsv (s4_resultset_t *set, xmms_fetch_spec_t *spec);
                      ...
                      const gchar *fetch_key, gint fetch_flags)
{
	...
	ret = s4_query (session->trans, spec, cond);
                                         ...
                                         const gchar *source)
{
	...
	s4_add (session->trans, "song_id", song_id, key, new_prop, source);
	s4_val_free (song_id);
...
xmms_medialib_get_new_id (xmms_medialib_session_t *session)
{
	...
	set = s4_query (session->trans, fs, cond);
                            ...
                            xmmsv_coll_t *coll, xmms_fetch_info_t *fetch)
{
	...
	ret = s4_query (session->trans, fetch->fs, cond);
	s4_cond_free (cond);
