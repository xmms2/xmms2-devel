...
$(foreach m, $(MODULE), $(eval $(OUTPUT_PRG)/$($(m)_PROGRAM)_OBJS:=$(foreach o, $($(m)_OBJS),$(OBJDIR)/$(o))))

# This produces the target libraries objects.
$(foreach m, $(MODULE), $(eval $(OUTPUT_LIB)/$($(m)_LIBRARIES)_OBJS:=$(foreach o, $($(m)_OBJS),$(OBJDIR)/$(o))))

# This produces the target plugin objects.
$(foreach m, $(MODULE), $(eval $(OUTPUT_PLUG)/$($(m)_PLUGIN)_OBJS:=$(foreach o, $($(m)_OBJS),$(OBJDIR)/$(o))))
...
$(LIBRARIES): 
	$(LIBTOOL) --mode=link $(CC) -rpath /usr/lib -export-dynamic -version-info 4:0:1 $(LINKFLAGS) $($@_LINKFLAGS) $($@_OBJS:.o=.lo) -o $@

$(PROGRAMS): 
	$(LIBTOOL) --mode=link $(CC) -o $@ $($@_OBJS:.o=.lo) $(LINKFLAGS) $($@_LINKFLAGS) 

$(PLUGINS): 
	$(LIBTOOL) --mode=link $(CC) -module -o $@ $($@_OBJS:.o=.lo) $(LINKFLAGS) $($@_LINKFLAGS)
	
builddir/%.o: src/%.c 
	$(LIBTOOL) --mode=compile $(CC) $(DEPEND) $(CFLAGS) $($@_CFLAGS) -c -o $@ $<
	@sed 's,^$(notdir $@),$@,' < .depend-tmp > .depend-$(subst /,_,$@) # Why oh why is GCC stupid?
	@rm .depend-tmp
